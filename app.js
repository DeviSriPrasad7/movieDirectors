...
app.get("/directors/", async (request, response) => {
  const getDirectorsQuery = `
    ...
    FROM
      director;`;
  const dirQuery = await database.all(getDirectorsQuery);
  response.send(
    dirQuery.map((eachDirector) => convertDirectorDbObjectToResponseObject(eachDirector))
  );
});
...
app.get("/directors/:directorId/movies/", async (request, response) => {
  ...
  const getDirMovQuery = `
    ...
    WHERE
      director_id = '${directorId}';`;
  const moviesArray = await database.all(getDirMovQuery);
